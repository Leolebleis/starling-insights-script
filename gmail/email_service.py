...
def create_message(sender, to, subject, html, attachment):
    ...
    Args:
      ...
      attachment: The pdf generated by starling.
    ...
    msg = MIMEApplication(attachment, _subtype="pdf")

    msg.add_header("Content-Disposition", "attachment", filename="bank_statement_{date}.pdf".format(date=today))
    message.attach(msg)
...
def create_html_body(data):
    net_spends = get_net_spends(data["net_spends"])
    net_spend_labels = net_spends[0]
    net_spend_data = net_spends[1]

    balance = get_balance(data["balance"])

    first_date = datetime.strptime(net_spend_labels[0], "%Y-%m")
    first_date = calendar.month_name[first_date.month] + str(first_date.year)

    sum_net_spend = sum(net_spend_data)
    ...
    current_month_savings = "{percent} {superlative}".format(percent=percent,
                                                             superlative="more. Keep up the good work! :)"
                                                             if better_than_last_month
                                                             else "less. Keep trying! Things will work out :)")

    highest_spend_category_per_period = get_highest_spend_category_per_period(data["highest_spend_category_per_period"])
    highest_spend_category_per_period_labels = highest_spend_category_per_period[0]
    highest_spend_category_per_period_data = highest_spend_category_per_period[1]

    spend_by_category_this_month = get_spend_by_category_this_month(data["spends_per_category_for_this_month"])
    spend_by_category_this_month_labels = spend_by_category_this_month[0]
    spend_by_category_this_month_data = spend_by_category_this_month[1]

    spend_per_party_this_month = get_spend_per_party_this_month(data["counter_party_current_month"]["breakdown"])
    spend_per_party_this_month_labels = spend_per_party_this_month[0]
    spend_per_party_this_month_data = spend_per_party_this_month[1]

    first_savings_goal = data["savings_goals"]["savingsGoalList"][0]

    savings_goal = first_savings_goal["name"]
    savings_goal_amount_left = (first_savings_goal["target"]["minorUnits"] - first_savings_goal["totalSaved"]["minorUnits"])/100
  ...
  <body style="font-family: &quot;Catamaran&quot;, sans-serif;background-color: #d8dbdb;font-size: 18px;max-width: 800px;margin: 0 auto;padding: 2%;color: #565859;">
    <div id="wrapper" style="background: #f6faff;">
      ...
      <div class="two-col" style="float: left;width: 46%;padding: 2%;">
        ...
        <p style="text-align: justify;">Net spend is the net of any money going in or out of my current account. 
    ...
    <img src="https://quickchart.io/chart?width=500&height=300&c=
      ...
      datasets: [{ label: 'Expenses per category over time',
      backgroundColor:'rgba(62, 149, 205, 1)',
      data: ${highest_spend_category_per_period_data}
      , }] }}" alt="Highest spend category over time" style="max-width: 100%;">
        <p style="text-align: justify;">This is the category for which you've spent the most money, excluding rent. 
        If you see this reducing over time, then that's great! You're controlling yourself more. 
        If not... well, it happens.</p>
      </div>
      <div class="two-col" style="float: left;width: 46%;padding: 2%;">
        <h2>Where you spent the most</h2>
        <img src="https://quickchart.io/chart?width=500&height=300&c=
        {type:'bar', 
        data: { labels: ${spend_per_party_this_month_labels}, 
        datasets: [{ label: 'Expenses per category this month',
        backgroundColor:'rgba(62, 149, 205, 1)',
        data: ${spend_per_party_this_month_data}, }] }}" alt="counter party where I've spent the most this month" style="max-width: 100%;">
        <p style="text-align: justify;">
      ...
      <div class="two-col" style="float: left;width: 46%;padding: 2%;">
        ...
        <img src="https://quickchart.io/chart?width=500&height=300&c=
          ...
          datasets: [{ label: 'Expenses per category this month',
          backgroundColor:'rgba(62, 149, 205, 1)',
          data: ${spend_by_category_this_month_data}, }] }}" 
          style="max-width: 100%;" alt="Spend by category this month">
        <p style="text-align: justify;">
    ...
    """).safe_substitute(this_months_net_spend=net_spend_data[-1],
                         ...
                         spend_per_party_this_month_labels=spend_per_party_this_month_labels,
                         spend_per_party_this_month_data=spend_per_party_this_month_data,
